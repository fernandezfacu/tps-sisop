...
static int
sys_change_prio(envid_t env_id, int prio)
{
	...
	struct Env *env;
	int e;
	if ((e = envid2env(env_id, &env, 1)))
		return e;  // can´t change prio of environment that is not curenv or an immediate child of curenv

	if (env->env_priority > prio)
		return -E_INVAL;

	env->env_priority = prio;
	env->env_priority_losses++;
	if (prio > env->env_min_prio)
		env->env_min_prio = prio;
...
sys_exofork(void)
{
	...
	newenv->env_runs_since_prio_change = 0;
	newenv->env_priority_losses = 0;
	newenv->env_min_prio = curenv->env_priority;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_get_env_prio:
		return sys_get_env_prio();
	case SYS_change_prio:
