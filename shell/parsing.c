...
expand_environ_var(char *arg)
{
	...
	if ((idx = block_contains(arg, '$')) == 0) {
		if (arg[1] ==
		    '?') {  // si se recibe "$?____", se ignora lo posterior a "$?".
			...
			if (!env_arg) {
				...
				if (strlen(env_arg) > strlen(arg))
					arg = realloc(arg, strlen(env_arg));
				strcpy(arg, env_arg);
			}
		}
	}
...
parse_exec(char *buf_cmd)
{
	...
	while (buf_cmd[idx] != END_STRING) {
		...
		if (strlen(tok) >
		    0) {  // para soportar comandos con múltiples espacios entre argumentos.
			c->argv[argc++] = tok;
		} else {
			free(tok);
		}
